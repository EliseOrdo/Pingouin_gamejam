...
class Cible:
    ...
    def touche_cible(self, pingouin):
        ...
        if pingouin.touche_truc(self):
            ...
            #Met le pinguoin au centre de la cible (pas sûr ça, T)
            #gauche
            if pingouin.x<=self.x :
                screen.blit(imcache, (pingouin.x-40, pingouin.y))
                pg.display.update(pg.Rect(pingouin.x-40, pingouin.y, 40, 40))
                
            if pingouin.y<=self.y :
                screen.blit(imcache, (pingouin.x, pingouin.y-40))
                pg.display.update(pg.Rect(pingouin.x, pingouin.y-40, 40, 40))
            else :
                screen.blit(imcache, (pingouin.x, pingouin.y))
...
imcache = pg.image.load("dessins/snow.png").convert_alpha()

# Fait les listes

liste_murs = [Mur(random.randint(0, 800), random.randint(0, 1000), (119, 129)) for j in range(random.randint(1, 3))]
...
while runningf:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            mouvements += 1
            liste_pingouins[0].rotation(event.key)
    ...
    for ciblind in liste_cibles:
        ...
        if ciblind.anim:
            ...
            pg.display.update((ciblind.x, ciblind.y, 40, 40))
            time.sleep(0.3)
            screen.blit(imcache, (ciblind.x, ciblind.y))
            pg.display.update((ciblind.x, ciblind.y, 40, 40))
            ciblind.anim = False
    ...
    for pingind in range(len(liste_pingouins)):
        if not liste_pingouins[pingind].cache:
            ...
            print(liste_pingouins[pingind].x, liste_pingouins[pingind].y)
